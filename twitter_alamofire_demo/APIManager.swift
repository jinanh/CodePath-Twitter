...
class APIManager: SessionManager {
    ...
    static let consumerKey = "WlZrJEqsYmu0pnI0Wcuc3ZIcg"
    static let consumerSecret = "5lfEYJZVAQP0ZAvWcfgQo5W9UsWLv1s1fOoEh5pfa0rEKhnfHH"
        ...
        oauthManager.authorize(withCallbackURL: callbackURL, success: { (credential, _response, parameters) in
            ...
            self.getCurrentAccount(completion: { (user, error) in
                if let error = error {
                    ...
                    print("Welcome \(String(user.name!) ?? "")")
    ...
    //Reply Tweet
    func replyTweet(with text: [String: Any], completion: @escaping (Tweet?, Error?) -> ()) {
        let urlString = "https://api.twitter.com/1.1/statuses/update.json"
